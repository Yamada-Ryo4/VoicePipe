...
- **直接下载**: [VoicePipeSetup.exe](https://github.com/Yamada-Ryo4/VoicePipe/releases/download/Release/VoicePipeSetup.exe) (包含运行环境及虚拟声卡驱动，双击一键安装)
- **使用方法**:
  1. 安装后打开 VoicePipe。
...
- **Pull 模型混音引擎**：基于 `IWaveProvider` 的 Pull 架构，`WaveOutEvent` 按设备时钟精确拉取混音数据，从根本上消除了推送模型导致的卡顿与延迟问题。
- **高保真音质**：支持 16-bit, 24-bit 以及 32-bit 浮点麦克风输入格式，自动将多声道降混至立体声。采用 Catmull-Rom 三次样条重采样与 tanh 软限幅，确保高频细节与动态余量。
- **反馈环路保护**：音频源列表自动过滤 VoicePipe 自身进程，防止用户误选导致无限噪音循环。
...
1. **LoopbackCapturer**：通过 COM 底层接口 (`IAudioClient`, `ActivateAudioInterfaceAsync`) 发起 Per-Process Loopback 流，精准捕获目标进程 (PID) 的音频。所有已激活的会话按 PID 缓存在 `PipelineManager` 中，切换音频源时直接复用，避免 Windows API 重复激活失败。
2. **MicCapturer**：使用 NAudio 的 `WasapiCapture` 捕获麦克风输入，并将 PCM 数据统一标准化为 32-bit IEEE Float 格式，完美兼容 16/24/32-bit 的各类专业声卡。
3. **AudioMixEngine**：实现 `IWaveProvider` 接口，由 `WaveOutEvent` 按设备时钟拉取数据（Pull 模型）。通过线程安全的双 RingBuffer 读取音频流，Catmull-Rom 三次样条重采样匹配 44.1kHz，tanh 软限幅保护动态范围，并生成 UI 渲染所需的波形数据。
4. **VirtualMicWriter**：接收混合完成的 32-bit float PCM 数据，使用 WASAPI 共享模式持续写入 VB-Cable 的 "CABLE Input" 端点。
5. **PipelineManager**：管线协调中心。维护 `Dictionary<int, LoopbackCapturer>` 缓存池，通过闭包检查 `_currentPid` 确保仅活跃 PID 的数据参与混音。`StopAsync` 只停输出和麦克风，loopback 保持后台运行以支持即时恢复。
